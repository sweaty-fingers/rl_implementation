...
from src.common.type_aliases import AtariResetReturn, AtariStepReturn
...
class StickyActionEnv(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    ...
    def __init__(self, env: gym.Env, action_repeat_probability: float) -> None:
        ...
        assert env.unwrapped.get_action_meanings()[0] == "NOOP"  # type: ignore[attr-defined] NOOP?
    ...
    def step(self, action: int) -> AtariStepReturn:
        if self.np_random.random() >= self.action_repeat_probability: # action_repeat_probability 이상의 확률로 랜덤하게 액션을 반복함.
...
class NoopResetEnv(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    ...
    :param env: Environment to wrap
    :param noop_max: Maximum value of no-ops to run # 최대 몇번의 noop를 실행할 것인지
    """
...
class EpisodicLifeEnv(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    ...
    def reset(self, **kwargs) -> AtariResetReturn:
        ...
        if self.was_real_done:
            ...
            if terminated or truncated: # 목숨을 다 잃거나, timeout으로 인해 game over 되었을 때
                obs, info = self.env.reset(**kwargs)
