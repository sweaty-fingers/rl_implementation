...
from src.common.preprocessing import get_action_dim, is_image_space, maybe_transpose, preprocess_obs
...
from src.common.utils import get_device, is_vectorized_observation, obs_as_tensor
...
class BasePolicy(BaseModel, ABC):
    ...
    def _dummy_schedule(progress_remaining: float) -> float:
        ...
        progress_remaining: 학습 진행률을 나타내는 값
        """
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if use_sde: # ?? State Dependent Exploration
            dist_kwargs = {
                "full_std": full_std,
                "squash_output": squash_output,
                "use_expln": use_expln,
        ...
        # Action distribution
        self.action_dist = make_proba_distribution(action_space, use_sde=use_sde, dist_kwargs=dist_kwargs) # 액션 클래스에 맞는 distribution 클래스 인스턴스 반환
    ...
    def _get_constructor_parameters(self) -> dict[str, Any]:
        ...
        default_none_kwargs = self.dist_kwargs or collections.defaultdict(lambda: None)  # type: ignore[arg-type, return-value] # self.dist_kwargs 가 None 일 경우 디폴트 딕셔너리 반환
