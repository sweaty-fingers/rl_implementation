...
# functools.singledispath 데코레이터를 사용하여 기본 함수를 작성한 후
# functools.register 데코레이터를 사용하여 첫번째 인자의 타입에 따른 함수를 작성 
        ...
        with zipfile.ZipFile(file) as archive:
            ...
            for file_path in pth_files:
                with archive.open(file_path, mode="r") as param_file:
                    ...
                    # fixed in python >= 3.7
                    # 파일 내용을 읽어와서 BytesIO 객체에 저장
                    # zip 파일에 저장된 파일은 그 자체로는 seekable 하지 않기 때문에
                    # 이미 파일을 읽는 포인터의 위치가 넘어가면 앞으로 되돌아갈 수 없음.
                    # 그러나 파이토치 데이터를 로드하기 위해서는 seekable 해야함.
                    # 따라서 파일 내용을 읽어와서 BytesIO 객체에 저장 (메모리에 저장)
                    # 그러면 파일 포인터의 위치를 임의로 조작할 수 있음.
